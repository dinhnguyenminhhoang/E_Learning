...
export function useDataTable<TData>(props: UseDataTableProps<TData>) {
  ...
  const immediateSetFilterValues = React.useCallback(
  ...
  const debouncedCallbackSetFilterValues = useDebouncedCallback((values: typeof filterValues) => {
    void setPage(1);
    void setFilterValues(values);
  }, debounceMs);

  // If debounceMs <= 0 we want immediate updates (no setTimeout). Use direct callback.
  const debouncedSetFilterValues = debounceMs > 0 ? debouncedCallbackSetFilterValues : immediateSetFilterValues;
