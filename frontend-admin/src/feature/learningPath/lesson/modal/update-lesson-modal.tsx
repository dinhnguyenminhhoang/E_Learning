...
export const UpdateLessonModal: React.FC<LessonModalProps> = ({ isOpen, onClose, mode, lesson }) => {
  ...
  const getData = useCallback(() => {
    if (mode === "edit" && lesson) {
      reset({
        ...
        blocksJson: lesson.blocks ? JSON.stringify(lesson.blocks, null, 2) : "",
      });
    } else {
      reset({
        title: "",
  ...
  const onSubmit = async (values: LessonFormValues) => {
    try {
      ...
      } else if (mode === "edit" && lesson?.id) {
        const payload: LessonUpdateBody = { ...(values as LessonUpdateBody) };

        await updateLesson({ body: payload, lessonId: lesson._id }).unwrap();
            ...
            {mode === "edit" && (
              <>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Blocks (JSON, optional)</FormLabel>
                      <FormControl>
                        <Textarea {...field} placeholder='[ { "block": "id", "exercise": "id", "order": 1 } ]' />
                      </FormControl>
                      <p className="text-muted-foreground text-sm">
                        Nếu chỉnh sửa blocks, dán JSON vào đây. Để trống để giữ nguyên.
                      </p>
                      <FormMessage />
                    </FormItem>
                  )}
