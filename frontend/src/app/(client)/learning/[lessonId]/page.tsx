...
export default function LearningPage() {
    ...
    try {
      ...
      if (response.code === 200 && response.data) {
        ...
        const blocksWithLock = blocksData.map((block: any, index: number) => {
          ...
          const isLocked =
            index > 0 && previousBlock && !previousBlock.isCompleted;
    ...
    try {
      ...
      if (response.code === 200) {
        ...
        const durationFromResponse =
          ...
          responseData?.lessonContent?.blockData?.duration;
  ...
  const startVideoHeartbeat = () => {
    console.log("Starting video heartbeat...");
    if (heartbeatIntervalRef.current) {
      clearInterval(heartbeatIntervalRef.current);
    }
    ...
    heartbeatIntervalRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (response.code === 200 && response.message?.data) {
          const { isCompleted, isLearned, progressPercentage } =
  ...
  const updateBlockProgress = (
    blockId: string,
    updates: Partial<BlockWithLock>
  ) => {
    setBlocks((prevBlocks) => {
      //Cập nhật thông tin cho block hiện tại (ví dụ: isCompleted = true)
      const newBlocksData = prevBlocks.map((block) =>
        block._id === blockId ? { ...block, ...updates } : block
      );

      //TÍNH TOÁN LẠI TRẠNG THÁI KHÓA (Re-calculate Locks)
      // Đây là bước còn thiếu gây ra lỗi của bạn
      return newBlocksData.map((block, index) => {
        // Block đầu tiên không bao giờ khóa
        if (index === 0) return { ...block, isLocked: false };

        // Lấy block liền trước trong danh sách MỚI (đã cập nhật)
        const prevBlock = newBlocksData[index - 1];

        // Logic khóa: Nếu block trước CHƯA completed -> Block này bị khóa
        // (Lưu ý: Giữ nguyên các thuộc tính khác, chỉ update isLocked)
        return {
          ...block,
          isLocked: !prevBlock.isCompleted,
        };
      });
    });
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500 hidden md:block mt-1">
              {blocks.length > 0 &&
                `Bài ${activeBlockIndex + 1} / ${blocks.length}`}
            </p>
          </div>
        </div>
                    ...
                    <div className="aspect-video bg-black">
                      <iframe
                        ...
                        src={
                          ...
                          "https://www.youtube.com/embed/GnecCts0msU"
                        }
                    ...
                    <div className="flex items-center justify-between gap-4 pt-4">
                      ...
                      <Button
                        ...
                        disabled={
                          ...
                          blocks[activeBlockIndex + 1]?.isLocked
                        }
                ...
                {blockVideoUrl && (
                  <div className="mb-6">
                    <VideoPlayer
                      ...
                      title={activeBlock.title}
                      type={
                        (activeBlock.type as string) === "media"
                          ? "media"
                          : "video"
                      }
                      onLoad={() => {
                      ...
                      {lessonContent.blockData?.examples &&
                        ...
                        lessonContent.blockData.examples.length > 0 && (
                          <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 md:p-8 border border-green-200 shadow-sm">
                            ...
                            <ul className="space-y-3">
                              {lessonContent.blockData.examples.map(
