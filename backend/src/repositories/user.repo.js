...
class UserRepository {
    constructor() {
        this.model = User;
        this.defaultSelect = '-password -security.refreshTokens -verification.twoFactorSecret';
        this.sensitiveFields = ['password', 'security.refreshTokens', 'verification.twoFactorSecret'];
    }
        ...
        try {
            ...
            if (validateEmails) {
                ...
                if (existing.length > 0) {
                    throw new Error(`Duplicate emails found: ${existing.map(u => u.email).join(', ')}`);
                }
            }
    ...
    async searchUsers(searchTerm, options = {}) {
        try {
            ...
            const users = await this.model
                ...
                .sort(includeScore ? { score: { $meta: "textScore" } } : { createdAt: -1 })
                .limit(limit)
                .skip(skip)
                .lean();
    ...
    async updateById(userId, updateData, options = {}) {
        try {
            ...
            await this.logActivity(userId, 'user_updated', { fields: Object.keys(safeUpdateData) });
    ...
    async updatePortfolio(userId, action, portfolioId) {
        try {
            ...
            await this.logActivity(userId, 'portfolio_updated', { action, portfolioId });
            return user;
        } catch (error) {
            ...
            const stats = await this.model.aggregate([
                { $match: matchStage },
                {
                    $group: {
                        ...
                        verified: {
                            $sum: { $cond: [{ $eq: ["$verification.emailVerified", true] }, 1, 0] }
                        },
                        sellers: {
